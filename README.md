...
Here is an example of how to use the script employing the system command in RStudio:
...
Apply script_scanmir to the data from [Larriba et al., 2023](https://pubmed.ncbi.nlm.nih.gov/37245055/). Assig a unique code to each set of mRNA targets, forming isoTargets groups (canonical miRNA and other isomiR regulating the same mRNA share the same code). Then, pooling the reads of all canonical miRNA and other isomiR within each isoTargets group across all samples and apply differential expression analysis using [DESeq2](https://bioconductor.org/packages/release/bioc/html/DESeq2.html) (differential expression analysis at the isoTargets level).
